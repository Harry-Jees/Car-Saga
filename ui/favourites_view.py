"""
Favourites View for Car Saga.
Shows cars the user has bookmarked.
"""
...
class FavouritesView(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        cars = FavouriteService.get_favourites(user_id)

        top_bar = ctk.CTkFrame(self, fg_color="#1e1e1e", corner_radius=10)
        top_bar.pack(fill="x", padx=10, pady=(4, 8))
        ctk.CTkLabel(top_bar, text=f"Favourites ({len(cars)})", font=ctk.CTkFont(size=16, weight="bold"), text_color="#e0e0e0").pack(side="left", padx=14, pady=10)
        ctk.CTkLabel(top_bar, text="Cars you have bookmarked.", font=ctk.CTkFont(size=12), text_color="#616161").pack(side="left", padx=4)

        if not cars:
            empty = ctk.CTkFrame(self, fg_color="transparent")
            empty.pack(expand=True, pady=60)
            ctk.CTkLabel(empty, text="No favourites yet.", font=ctk.CTkFont(size=18, weight="bold"), text_color="#9e9e9e").pack(pady=(0, 6))
            ctk.CTkLabel(empty, text="Click ♥ on any car in the catalogue to save it here.", font=ctk.CTkFont(size=13), text_color="#616161").pack(pady=(0, 16))
        ...
        for car in cars:
            ...
            ctk.CTkLabel(center, text=f"Added: {str(car.get('added_on', ''))[:16]}", font=ctk.CTkFont(size=11), text_color="#616161", anchor="w").pack(fill="x")
            ...
            ctk.CTkButton(right, text="Remove", width=96, height=28, fg_color="#3a2a2a", text_color="#cf6679", hover_color="#4a2a2a", command=lambda c=cid: self.remove_fav(c)).pack(pady=2)

    def remove_fav(self, car_id):
        FavouriteService.remove_favourite(self.app.current_user["id"], car_id)
        self.refresh()
